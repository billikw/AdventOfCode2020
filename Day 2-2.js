...
function executeChecks(passwordArray) {
    ...
    for(i=0; i<sourceArr.length; i++){
        checkPositions(min[i],max[i],character[i],password[i]);
    }
...
function getMin(passwordArray){
    ...
    for (i=0; i<passwordArray.length; i++){
        let iteration = passwordArray[i].split("-");
        let parsed = parseInt(iteration[0]);
        min.push(parsed);
    }
}
function getMax(passwordArray){

    for (i=0; i<passwordArray.length; i++){
        let iteration = passwordArray[i].split(" ");
        iteration = iteration[0].split("-");
        let parsed = parseInt(iteration[1]);
        max.push(parsed);
    }
}
function getChar(passwordArray){

    for (i=0; i<passwordArray.length; i++){
        let iteration = passwordArray[i].split(" ");
        iteration = iteration[1].split(":");
        character.push(iteration[0]);
    }
}
function getPassword(passwordArray){
...
function checkPositions(first,last,character,password){
    
    let char1 = password.charAt(first-1);
    let char2 = password.charAt(last-1);
